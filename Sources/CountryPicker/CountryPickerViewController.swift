...
public final class CountryPickerViewController: UIViewController {
    ...
    func filter(for query: String?) {
        if query?.count == 0 {
            ...
            filteredCountries = countries
                .filter { $0.localizedName.localizedLowercase.contains(text.localizedLowercase) || $0.phoneCode.localizedLowercase.contains(text.localizedLowercase) || $0.isoCode.localizedLowercase.contains(text.localizedLowercase) }
        }
        tableView.reloadData()
    }
}
